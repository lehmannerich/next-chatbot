  ...
  for (let i = 0; i < essays.length; i++) {
    ...
    for (let j = 0; j < section.chunks.length; j++) {
      ...
      if (error) {
        ...
        // show progress essay scraped from all essays
        console.log("saved:", i, j, "total:", essays.length);
      }
...
(async () => {
  const book: PGJSON = JSON.parse(fs.readFileSync("scripts/new-testament.json", "utf8"));
